...
class tictoPage {
	...
	validarValoresInseridosJSON() {
		...
		cy.get(el.GRID)
			...
			.find(el.linhaGRID)
			.invoke("text")
			.then(($id) => {cy.log(`Estamos trabalhando com esse Id ${$id}`)});
		cy.contains(el.GRID).contains(usuario.nome).should("be.visible");
		cy.contains(el.GRID).contains(usuario.email).should("be.visible");
	}
}
