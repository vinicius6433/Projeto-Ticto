...
class tictoPage {
	...
	preencherSenha(senha) {
		if(senha == 'senha com mais de 255 caracteres'){
			senha = "Maecenas ipsum velit, consectetuer eu, lobortis ut, dictum at, dui. In rutrum. Sed ac dolor sit amet purus malesuada congue. In laoreet, magna id viverra tincidunt, sem odio bibendum justo, vel imperdiet sapien wisi sed libero. Suspendisse sagittis ultrices augue. Mauris metus. Nunc dapibus tortor vel mi dapibus sollicitudin. Etiam posuere lacus quis dolor. Praesent id justo in neque elementum ultrices. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos hymenaeos. In convallis. Fusce suscipit libero eget elit. Praesent vitae arcu tempor neque lacinia pretium. Morbi imperdiet, mauris ac auctor dictum, nisl ligula egestas nulla, et sollicitudin sem purus in lacus. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Morbi gravida libero nec velit. Morbi scelerisque luctus velit. Etiam dui sem, fermentum vitae, sagittis id, malesuada in, quam. Proin mattis lacinia justo. Vestibulum facilisis auctor urna. Aliquam in lorem sit amet leo accumsan lacinia. Integer rutrum, orci vestibulum ullamcorper ultricies, lacus quam ultricies odio, vitae placerat pede sem sit amet enim. Phasellus et lorem id felis nonummy placerat. Fusce dui leo, imperdiet in, aliquam sit amet, feugiat eu, orci. Aenean vel massa quis mauris vehicula lacinia. Quisque tincidunt scelerisque libero. Maecenas libero. Etiam dictum tincidunt diam. Donec ipsum massa, ullamcorper in, auctor et, scelerisque sed, est. Suspendisse nisl. Sed convallis magna eu sem. Cras pede libero, dapibus nec, pretium sit amet, tempor quis, urna."

			senha += " Morbi a metus. Phasellus enim erat, vestibulum vel, aliquam a, posuere eu, velit. Nullam sapien sem, ornare ac, nonummy non, lobortis a, enim. Nunc tincidunt ante vitae massa."
		}
	...
	validarMensagensErro(nome, email, senha) {
		switch (nome) {
			case "nome invalido":
				cy.get(el.msgErro).contains(msgInvalidas.nomeInvalido).should("be.visible");
				break;
			case "nome obrigatorio":
				cy.get(el.msgErro).contains(msgInvalidas.nomeCampoObrigatorio).should("be.visible");
				break;
			default:
				break;
		}
	...
	validarMensagem(msg){
		switch (msg) {
			case "Usuário cadastrado com sucesso.":
				...
				cy.cadastrarUsuario(el.usuario,el.emailJson)
				cy.get(el.GRID).contains(NovoUsuario.nome).parent().find(el.linhaGRID).invoke('text').then(($id) => {
					cy.cadastrarId($id)
				})
				break;
			default:
	...
	validarUsuarioCadastrado(){
		cy.get('tr').contains(NovoUsuario.nome).should('be.exist')
		

		// cy.get(el.GRID).contains(NovoUsuario.nome).parent().find(el.btnGRID).click();
	}
	validarValoresInseridosJSON(){
		cy.wait(2000)
		cy.contains(NovoUsuario.id).should('be.exist')
		cy.contains(NovoUsuario.id).parent().contains(NovoUsuario.nome).should('be.visible')
		cy.contains(NovoUsuario.id).parent().contains(NovoUsuario.email).should('be.visible')
	}
}
