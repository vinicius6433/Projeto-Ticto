...
class tictoPage {
	...
	validarValoresInseridosJSON(){
		cy.wait(2000)
		cy.contains(usuario.id).should('be.exist')
		cy.contains(usuario.id).parent().contains(usuario.nome).should('be.visible')
		cy.contains(usuario.id).parent().contains(usuario.email).should('be.visible')
	}
}
