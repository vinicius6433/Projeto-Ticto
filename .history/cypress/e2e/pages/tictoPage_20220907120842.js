...
const idUsuario = require("../../fixtures/idUsuario.json")
const el = {
	url: "",
	body: "body",
...
class tictoPage {
	...
	validarMensagem(msg){
		switch (msg) {
			case "Usuário cadastrado com sucesso.":
				...
				cy.get(el.GRID).contains(usuario.nome).parent().find(el.linhaGRID).invoke('text').then(($id) => {
					cy.cadastrarId($id)
				})
				break;
			default:
	...
	validarValoresInseridosJSON(){
		cy.wait(2000)
		cy.readFile('../../fixtures/idUsuario.json').then(())
		cy.contains(idUsuario.id).should('be.exist')
		cy.contains(idUsuario.id).parent().contains(usuario.nome).should('be.visible')
		cy.contains(idUsuario.id).parent().contains(usuario.email).should('be.visible')
	}
}
