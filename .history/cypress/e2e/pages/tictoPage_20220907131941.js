...
const emailUsuario = require("../../fixtures/emailUsuario.json");
const usuario = require("../../fixtures/nomeUsuario.json");
...
class tictoPage {
	...
	preencherNome(nome) {
		...
		el.nomeJson = nome;
		cy.writeFile("../../fixtures/nomeUsuario.json", {
			nome: nome,
		});
	}
	preencherEmail(email) {
		...
		el.emailJson = email;
		cy.writeFile("../../fixtures/emailUsuario.json", {
			nome: email,
		});
	}
	...
	btnCadastrar() {
		...
		cy.get(el.GRID)
			...
			.invoke("text")
			.then(($id) => {
				cy.writeFile("../../fixtures/id.json", {
					id: $id,
				});
				cy.log('Id adicionado ao json')
			});
	}
	btnExcluir(){
	...
	validarValoresInseridosJSON() {
		...
		cy.get(el.GRID)
			...
			.then(($id) => {
				...
				cy.contains($id).parent().contains(usuario.nome).should("be.visible");
				cy.contains($id).parent().contains(emailUsuario.nome).should("be.visible");
